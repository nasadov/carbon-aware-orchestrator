...
# ---------------------------------------------------------------- strong 7-seed (RQ5)
try:
    per_seed = list(csv.DictReader(open(E / "replication_ci" / "part1_strong_real2022_v3_per_seed.csv")))  # v3 = German-basin strong fleet (Depth-1.5 fold 2026-07-07)
...
# ---------------------------------------------------------------- share x slack regime sweep (banked; W4 fold)
try:
    rows: list[dict] = []
    for d in ("regime_flexfrac", "regime_slack"):
        rows += list(csv.DictReader(open(E / d / "cells.csv")))
    n_cert = sum(1 for r in rows if float(r["cert_rate"]) == 1.0)
    add("PNsweepCells", f"{n_cert}/{len(rows)}")
    slack_rows = [r for r in rows if r["knob"] == "slack"]
    add("PNsweepReliefLo", f"{min(float(r['stress_kwh_avoided_mean']) for r in slack_rows):.1f}")
    add("PNsweepReliefHi", f"{max(float(r['stress_kwh_avoided_mean']) for r in slack_rows):.1f}")
    share = [float(r["flexible_pct"]) for r in rows if r["knob"] == "flex_frac"]
    add("PNsweepShareLo", f"{min(share):.0f}")
    add("PNsweepShareHi", f"{max(share):.0f}")
except Exception as exc:  # noqa: BLE001
    add("PNsweepMISSING", f"ERROR {exc}")

# ---------------------------------------------------------------- D1: US per-basin certification (Eq. 1)
try:
    us = json.load(open(E / "us_perbasin" / "us_perbasin_digest.json"))
    a, k = us["suite_a"]["per_method_o1"], us["suite_a"]["k"]
    add("PNusPBwindows", f"{k}")

    def _pb(m: str) -> str:  # per-basin Eq.(1) certified count over the K randomized US windows
        return f"{a[m]['per_basin_cert']}/{k}"

    add("PNusPBEnvelopeAgg", _pb("no_harm_flex"))                    # 2/24 (aggregate-guarded leaks)
    add("PNusPBGuarded", _pb("no_harm_flex_pbg"))                    # 24/24
    add("PNusPBGuardedCtl", _pb("no_harm_search_control_pbg"))       # 24/24
    add("PNusPBCarbonGreedy", _pb("carbon"))                         # 1/24
    add("PNusPBWaterGreedy", _pb("water_scarcity"))                  # 0/24
    add("PNusPBWaterWise", _pb("waterwise@0.5"))                     # 2/24
    baseline_keys = [m for m in a if m not in
                     ("no_harm_flex", "no_harm_flex_pbg", "no_harm_search_control",
                      "no_harm_search_control_pbg", "packing")]
    add("PNusPBBaselineMax", f"{max(a[m]['per_basin_cert'] for m in baseline_keys)}/{k}")
    add("PNusPBGuardedWilsonLo", f"{a['no_harm_flex_pbg']['wilson95'][0]:.2f}")   # 0.86
    add("PNusPBGuardedCarbonMax", sgn(a["no_harm_flex_pbg"]["carbon_delta_pct_max"], 2))  # <= 0
    add("PNusPBGuardCostMean", "+0.45")  # paired pp vs aggregate-guarded envelope (D1 REPORT sec. 2)
    grad = json.load(open(E / "us_perbasin" / "realized_gradient.json"))["gradient"]
    add("PNusBasinGradient", f"{grad['o1_total_ratio']:.0f}")        # 62x AZPS/BPAT
    az = json.load(open(E / "us_perbasin" / "runs" / "us4_az_seed42" / "case.json"))
    wg = az["methods"]["water_scarcity"]["o1"]["per_basin_delta_pct"]
    add("PNusWgAZPS", sgn(wg["AZPS"]))                               # -44.1
    add("PNusWgERCO", sgn(wg["ERCO"]))                               # +152.0
except Exception as exc:  # noqa: BLE001
    add("PNusPerBasinMISSING", f"ERROR {exc}")
